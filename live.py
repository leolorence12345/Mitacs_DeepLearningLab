...
img = cv2.imread(img)
res = model(img)
res_plotted = res[0].plot(labels=0,pil=1,masks=1,boxes=0)
masks = res[0].masks
points = np.array(masks.xyn[0])
print(masks.xyn)
...
# path = 'D:\\mitacs_blender\\runs\\detect\\predict2\crops\\Disk_1\\output_video12.jpg'
# img = cv2.imread(path)
# imgContour = img.copy()
